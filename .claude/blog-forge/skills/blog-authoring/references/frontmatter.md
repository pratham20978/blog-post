...
category: networking
series: tcp-on-lossy-links   # series members only; both keys or neither
series_position: 2
tier: L2
...
| `series` | The series slug, lowercase and hyphenated, matching `posts/<series-slug>/`, which contains `plan.md` and `context.md`. Omit on a standalone post. |
| `series_position` | The part number, an integer starting at 1. |
...
`series` and `series_position` **appear together or neither appears**. A series with no position cannot be ordered, and a position with no series belongs to nothing. The platform parser reads both and uses them to build the series navigation, so a mismatched pair is a rendering bug rather than a cosmetic one. `references/series-planning.md` decides the parts and their order.
